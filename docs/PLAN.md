...
| 디자인 시스템 | **anti-card**(`@freeive/anti-card`) 가이드를 따른다 | 타이포 12토큰 · **weight 3단계(400/500/600)** · zinc 7-step 앵커 + 단일 accent · dark/light 자동 · 모션 4단계. 핵심 원칙은 **"카드 안에 카드를 쌓지 않는다"** — 위계를 박스가 아니라 타이포와 여백으로 만든다. **색상은 전부 CSS 변수의 의미 역할명(`--bg`·`--text`·`--accent` …)으로만 참조**하고 리터럴을 쓰지 않는다. 유저 테마 선택(v2)이 토큰 블록 교체만으로 되게 하기 위함이다. |
...
- **테마 선택** — 기본은 OS 설정을 따르고(dark/light 자동), 유저가 명시적으로 고를 수 있게 한다. 색상 토큰이 이미 CSS 변수의 의미 역할명으로 분리돼 있어(§2) 토큰 블록 교체로 끝난다. 강제 라이트/다크 + accent 색 선택.
